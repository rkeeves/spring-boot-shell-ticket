Feature: allows creating, deleting and listing screenings
  ...
  Scenario: The admin can not create a screening which would end during a 10 minute long breaks after another screening in the same room
    Given the user types the "sign in privileged admin admin" command
    And the user types the "create screening Sátántangó Pedersoli \"2021-03-15 11:10\"" command
    When the user types the "create screening \"Spirited Away\" Pedersoli \"2021-03-15 09:00\"" command
    Then the next line of the output is "The existing movie start in the break period after the new screening in this room"
    When the user types the "list screenings" command
    Then the next line of the output is "Sátántangó (drama, 450 minutes), screened in room Pedersoli, at 2021-03-15 11:10"

  Scenario: a not signed in user cannot create or delete screenings
    Then the "create screening" shell command is not available
    Then the "delete screening" shell command is not available

  Scenario: a non-privileged user cannot create or delete screenings
    Given the user types the "sign up sanyi alma" command
    When the user types the "sign in sanyi alma" command
    Then the "create screening" shell command is not available
    Then the "delete screening" shell command is not available

  Scenario: deleting a movie causes associated screenings to be deleted
    Given the user types the "sign in privileged admin admin" command
    And the user types the "create screening \"Spirited Away\" Pedersoli \"2021-03-14 16:00\"" command
    And the user types the "list screenings" command
    And the next line of the output is "Spirited Away (animation, 125 minutes), screened in room Pedersoli, at 2021-03-14 16:00"
    When the user types the "delete movie \"Spirited Away\"" command
    And the user types the "list screenings" command
    Then the next line of the output is "There are no screenings"

  Scenario: deleting a room causes associated screenings to be deleted
    Given the user types the "sign in privileged admin admin" command
    And the user types the "create screening \"Spirited Away\" Pedersoli \"2021-03-14 16:00\"" command
    And the user types the "list screenings" command
    And the next line of the output is "Spirited Away (animation, 125 minutes), screened in room Pedersoli, at 2021-03-14 16:00"
