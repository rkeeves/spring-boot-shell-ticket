Feature: allow creating accounts and logging in
  ...
  Scenario: an signed in user cannot sign in
    When the user types the "sign in privileged admin admin" command
    Then the "sign in" shell command is not available

  Scenario: a not signed user cannot sign out
    Then the "sign out" shell command is not available
